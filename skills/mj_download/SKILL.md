...
| 스크립트 | 위치 | 역할 |
|----------|------|------|
| `check_login.py` | `skills/mj_login/` | 세션 JSON의 인증 쿠키 유효성 확인 |
| `login.py` | `skills/mj_login/` | 브라우저를 열어 수동 로그인, 세션을 JSON으로 저장 |
| `download.py` | `skills/mj_download/` | 세션 JSON으로 인증하여 오늘 이미지를 zip으로 다운로드 |
